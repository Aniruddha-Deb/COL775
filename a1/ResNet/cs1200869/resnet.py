...
def q2_part3(cifar10_path):
    ...
    first_batch = False
    feature_quantile_data = []
    def record_feature_quantiles(module, input_, output):
        if first_batch and not model.training:
            output_sorted = torch.sort(output[0].to(torch.device('cpu'))).values
            feature_quantile_data.append((
                output_sorted[0].item(), 
                output_sorted[63//5].item(), 
                output_sorted[63*4//5].item(),
                output_sorted[63].item()
            ))

    model.network[9].register_forward_hook(record_feature_quantiles)
    ...
    pickle.dump(feature_quantile_data, open('feature_quantile_data.pkl', 'wb'))
